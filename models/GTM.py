...
class TextEmbedder(nn.Module):
    ...
    def forward(self, category, color, fabric):
        ...
        word_embeddings = [
            torch.FloatTensor(x[0][1:-1]).mean(axis=0)
...
class GTM(pl.LightningModule):
    ...
    def encode_static(self, category, color, fabric, temporal_features, images):
    ...
    def encode_trends(self, gtrends):
        gtrend_encoding = self.gtrend_encoder(gtrends)
        pooled_gtrend = gtrend_encoding.mean(dim=0)
        return gtrend_encoding, pooled_gtrend

    def decode_from_embedding(self, static_feature_fusion, gtrend_encoding):
    ...
    def forward(self, category, color, fabric, temporal_features, gtrends, images):
        static_feature_fusion = self.encode_static(
            category,
            color,
            fabric,
            temporal_features,
        ...
        gtrend_encoding, _ = self.encode_trends(gtrends)
        return self.decode_from_embedding(static_feature_fusion, gtrend_encoding)
    ...
    def training_step(self, train_batch, batch_idx):
        item_sales, category, color, fabric, temporal_features, gtrends, images, *_ = train_batch
    ...
    def validation_step(self, val_batch, batch_idx):
        item_sales, category, color, fabric, temporal_features, gtrends, images, *_ = val_batch
        forecasted_sales, _ = self.forward(category, color, fabric, temporal_features, gtrends, images)
